...
%token COUNT SUM MAX MIN AS LIMIT
// non-keywords
...
%type <sv_orderby_list>  order_clause_list opt_order_clause
...
order_clause_list:
      order_clause
    {
        $$ = std::make_shared<OrderByList>(std::vector<std::shared_ptr<OrderBy>>{$1});
    }
